...
#undef GL_GET
	...
	//				setTextureFormat(TextureFormat::RGBA16,  GL_RGBA16UI, GL_RGBA_INTEGER, GL_UNSIGNED_SHORT);
				...
				// OpenGL ES has no glGetTexImage at any version, so texture read-back
				// has to go through an FBO + glReadPixels. The same fallback covers
				// blit when copy_image is unavailable -- it's only core in ES 3.2, and
				// WebGL 2.0 is ES 3.0.
		...
		void readTexture(TextureHandle _handle, void* _data, uint16_t _layer, uint8_t _mip) override
		{
			...
			else if (m_readBackFboSupported)
			{
				...
				// Compressed textures can't be attached to a framebuffer, so there is
				// no way to read them back without glGetCompressedTexImage.
...
#undef GLENUM
	...
	void RendererContextGL::submitBlit(BlitState& _bs, uint16_t _view)
	{
		...
		{
			// copy_image is only core in OpenGL ES 3.2. Fall back to a framebuffer
			// copy, which is limited to uncompressed 2D regions.
			while (_bs.hasItem(_view) )
			{
				const BlitItem& bi = _bs.advance();
